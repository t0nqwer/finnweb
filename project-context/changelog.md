...
- Made the publish endpoint enforce the engine: `SitePublishingService` now runs `evaluateSiteQuality` at site stage over the draft pages plus `Site.themeConfig` and refuses to publish while any error-severity issue remains. The 400 body carries the full report (`quality`) so the builder can list every problem with its Thai `ownerMessage`, while `message` keeps the exact legacy string (`PUBLISH_UNRESOLVED_PLACEHOLDERS_IN_SECTION:{id}`, `PUBLISH_REQUIRED_FIELD_MISSING:{type}:{field}`) in the original precedence so existing clients and `sites.integration.test.ts` see no contract change.
- Added `GET /api/sites/:siteId/quality`, an on-demand report for the same engine without publishing — the data source for the builder panel.
- Deleted the two duplicate rule copies this replaced: the private `validatePublishContent` / `getRequiredFieldMissingForPublish` / `hasUnresolvedPlaceholders` block in `sites.service.ts` was dead after the publishing facade extraction, and the equivalent block in `site-publishing.service.ts` is now the engine call. Placeholder checks on page fields and `Section.name` moved into the engine so that coverage was not lost.
- Fixed the two defects the audit found, at the source rather than per template: `generateTemplate` now strips media props that resolve to an empty string (content packs set `logoUrl: ""`, so every generated site shipped an empty logo slot that the renderer painted as a blank tile), and all three themes now define `--line-height-base: 1.75` for the Thai minimum of 1.7. Added the missing hero `imageAlt` to the clinic content pack.
- All nine shipped blueprint x content-pack x theme combinations now score a clean 100, locked in by `template-quality.test.ts` as the acceptance gate for future templates.
- **Behaviour change to watch:** publish is now blocked by error-severity issues. Newly generated sites pass, but sites created before this change may hold empty media slots or lack a lead-capture section and will be refused until fixed. The owner-facing Thai message names the exact fix.
- Put the engine in front of the owner while they edit: `BuilderShell` imports the shared engine directly and evaluates the open page on every change, so the builder's verdict is the publish gate's verdict with no round trip. `GET /api/sites/:siteId/quality` also returns the site's `themeConfig`, fetched once on load, so the live panel runs the contrast and Thai typography rules too instead of quietly skipping what a page's own sections cannot answer. `QualityPanel` sits under the section list showing the score and each issue in Thai; clicking an issue selects the section it came from.
- A refused publish no longer collapses to one opaque string — `PublishQualityError` carries the server's site-wide report (duplicate slugs, theme contrast: things a single page cannot see) and the panel switches to it until the next edit, at which point the live page report is current again. Site-scoped issues do not offer click-to-select, because their section indexes belong to their own page rather than the one on screen.
- Closed the two tooling gaps the status check found: `apps/web` now has a `typecheck` script, so `pnpm typecheck` covers the largest app instead of skipping it, and it now declares `tsx`, so `pnpm --filter web test` runs at all. Its test glob also had to be quoted — `sh` does not expand `**` recursively, so nested test files were being skipped silently; the web suite went from 1 test discovered to 8.
- Added consumer Thai AI content fill: `POST /api/sites/:siteId/pages/:pageId/ai-content` asks DeepSeek to rewrite a page's copy and returns it as a *proposal* — nothing is written to the database, so AI copy is applied through the same section-update path as copy typed by hand.
- The fill is gated by the same engine rather than trusted: the model only ever sees an allowlist of copy keys (`EDITABLE_COPY_KEYS`), so a bad completion can produce bad wording but never a changed URL, colour, or structure; proposals that are English, empty, contain `{{ }}`, or match filler patterns are dropped before they are applied; the rewritten page is then scored and kept only when it introduces no new errors and does not lower the score. Otherwise the failing rules are fed back to the model for one more attempt, and failing that the original copy stands.
- Every fallback is explicit (`ai_disabled`, `ai_unavailable`, `no_usable_copy`, `not_better`) and surfaced in Thai in the builder, so an owner is never left guessing why the text did not change. With no `DEEPSEEK_API_KEY` the endpoint degrades to a no-op rather than failing.
- Consolidated the LLM integration the same way the rules were consolidated: `DeepSeekClient` in a new `AiModule` is now the only place the API talks to DeepSeek, and `AdminTemplateAiService` was rewired onto it, dropping its private copy of the fetch/parse/error handling. The client returns `{data, usedAi, reason}` and never throws, so every AI path is forced to have a deterministic fallback.
- Coverage: 46 engine + 10 template-gate + 21 generator + 5 admin validator + 10 AI-fill logic + 8 web tests pass; `pnpm typecheck`, `pnpm build`, and `pnpm lint` are unchanged in status (lint still carries one pre-existing error in `HighDesignMotion.tsx`, untouched by this work). `sites.integration.test.ts` could not be run here — it needs the remote dev database, which is unreachable from this machine.
